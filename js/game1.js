...
class plain //Класс звездалета
{
	...
	Collide(plain) //Условия столкновения звездалета с метеоритом
	{
		...
		if(this.y < plain.y + plain.image.height * scale && this.y + this.image.height * scale > plain.y) //Если есть столкновение по y .jpg
		{
	...
	Move(v, d)  
	{
		if(v == "x") //Двигаться по x
		{
			...
			//Откат изменений, если звездалет покинул экран
...
var player = new plain("../images/plain1.png", canvas.width / 2, canvas.height / 1.1, true); //Игрок
...
function GameOver() //Функция окончания игры
{
	...
	document.getElementById("PauseMenu").style.opacity=1; //Вызывается надпись "конец игры"
